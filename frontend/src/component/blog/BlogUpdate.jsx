...
class BlogUpdate extends Component {
    ...
    // CDM (Update öncelikle veriyi bul)
    componentDidMount() {
        BlogApiServices.blogServiceFindById(this.state.id).then((response)=>{
            const findBlogData=response.data;

            this.setState({
                header:findBlogData.header,
                content:findBlogData.content,
            })       
         }).catch(err=>{
            console.log(err)
         })
    ...
    blogUpdateSubmit = async (event) => {
        ...
        try {
            ...
            const response = await BlogApiServices.blogServiceUpdateById(this.state.id,blogDto);
            if (response.status == 200) {
                this.setState({
    ...
    render() {
        ...
        const { validationErrors, isRead, spinnerData, multipleRequest,id } = this.state;
        ...
        return (
            <React.Fragment>
                <h2 className="display-3 text-center text-uppercase shadow mt-5 mb-3 p-4">Blog Update</h2>
                <form className="create_form" method='post' autoComplete='true'>
                    <h3 className="display-3 text-center">{id}</h3>
                    {/* Header input */}
